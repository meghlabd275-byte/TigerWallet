...
import (
	...
	"github.com/johnfercher/maroto/pkg/maroto"
...
func (s *BulkService) processTokenBulkOperation(op *BulkOperation) {
	for i, tokenID := range op.IDs {
		...
		switch op.Action {
		case "activate":
			if err := s.db.Model(&Token{}).Where("id = ?", tokenID).Update("is_active", true).Error; err != nil {
				op.FailedCount++
		...
		case "deactivate":
			if err := s.db.Model(&Token{}).Where("id = ?", tokenID).Update("is_active", false).Error; err != nil {
				op.FailedCount++
		...
		case "verify":
			if err := s.db.Model(&Token{}).Where("id = ?", tokenID).Update("is_verified", true).Error; err != nil {
				op.FailedCount++
...
func (s *ExportService) generatePDF(jobID string, data []map[string]interface{}, title string) (string, error) {
	...
	for i := 0; i < maxRows; i++ {
		...
		m.AddRow(8, func(r *Row) {
			for _, col := range columns {
				...
				r.AddCell(len(col), val, props.Cell{Size: 7})
			}
		})
	}
	...
	filePath := fmt.Sprintf("%s/%s.pdf", s.exportDir, jobID)
...
func main() {
	log.Println("Starting TigerWallet Admin Bulk Operations Service...")
	
	config := LoadBulkConfig()
	...
	// Create services
	bulkService := NewBulkService(config, db, redisClient)
	exportService := NewExportService(config, db, redisClient)
	
	// Setup routes
	r := gin.Default()
	
	// Bulk operations
	r.POST("/api/v1/bulk", bulkService.CreateBulkOperation)
	r.GET("/api/v1/bulk/:id", bulkService.GetBulkOperationStatus)
	
	// Export
	r.POST("/api/v1/export", exportService.CreateExport)
	r.GET("/api/v1/export/:id", exportService.GetExportStatus)
	r.GET("/api/v1/export/:id/download", exportService.DownloadExport)
	...
	addr := ":" + config.Port
	log.Printf("Bulk operations service listening on %s", addr)
