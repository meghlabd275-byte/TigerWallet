/**
 * TigerAdmin C++ Core - Futures Domain Handler
 *
 * Header-only handler for the /futures admin domain backed by the admin/go
...
class FuturesHandler : public DomainHandler {
public:
    FuturesHandler() : DomainHandler("/futures") {}

    // PUT /api/v1/futures/{id}/status
    void register_status(Router& router) {
        router.put("/api/v1/futures/{id}/status",
