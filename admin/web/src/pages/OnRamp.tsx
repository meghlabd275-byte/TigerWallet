/**
 * TigerWallet Admin - OnRamp Management Page
 * CRUD + status + approve/reject for fiat->crypto onramp orders (mirrors /api/v1/onramp)
 */
...
export const OnRampPage: React.FC = () => {
  ...
  const [editing, setEditing] = useState<OnRampOrder | null>(null);
  const [rejectingId, setRejectingId] = useState<string | null>(null);
  const [rejectReason, setRejectReason] = useState('');
  const [formData, setFormData] = useState({ userId: '', userName: '', fiatCurrency: 'USD', cryptoAsset: 'BTC', fiatAmount: '', cryptoAmount: '', paymentMethod: 'card', provider: 'stripe' });
  ...
  const loadOrders = async () => {
    try { setLoading(true); setError(null); const res = await onRampAPI.getAll(); setOrders(res.data || []); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load onramp orders'); } finally { setLoading(false); }
  };

  const resetForm = () => { setFormData({ userId: '', userName: '', fiatCurrency: 'USD', cryptoAsset: 'BTC', fiatAmount: '', cryptoAmount: '', paymentMethod: 'card', provider: 'stripe' }); setEditing(null); };
  const openCreate = () => { resetForm(); setShowForm(true); };
  const openEdit = (o: OnRampOrder) => { setEditing(o); setFormData({ userId: o.userId, userName: o.userName, fiatCurrency: o.fiatCurrency, cryptoAsset: o.cryptoAsset, fiatAmount: String(o.fiatAmount), cryptoAmount: String(o.cryptoAmount), paymentMethod: o.paymentMethod, provider: o.provider }); setShowForm(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const payload = { ...formData, fiatAmount: Number(formData.fiatAmount), cryptoAmount: Number(formData.cryptoAmount) };
      if (editing) await onRampAPI.update(editing.id, payload); else await onRampAPI.create(payload);
      setShowForm(false); resetForm(); loadOrders();
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to save order'); }
  };

  const handleDelete = async (id: string) => { if (!confirm('Delete this onramp order?')) return; try { await onRampAPI.delete(id); loadOrders(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to delete order'); } };
  const handleStatusChange = async (id: string, status: Status) => { try { await onRampAPI.setStatus(id, status); loadOrders(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to update status'); } };
  const handleApprove = async (id: string) => { try { await onRampAPI.approve(id); loadOrders(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to approve order'); } };
  const handleReject = async () => { if (!rejectingId) return; try { await onRampAPI.reject(rejectingId, rejectReason); setRejectingId(null); setRejectReason(''); loadOrders(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed to reject order'); } };

  return (
    <div className="p-6" style={{ color: colors.text }}>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold" style={{ color: colors.text }}>OnRamp Management</h1>
          ...
          <div className="card-body">
            <form onSubmit={handleSubmit}>
              <div className="flex gap-4" style={{ flexWrap: 'wrap' }}>
                ...
                <div className="form-group"><label className="form-label">Payment Method</label><input className="form-input" value={formData.paymentMethod} onChange={(e) => setFormData({ ...formData, paymentMethod: e.target.value })} required style={{ backgroundColor: 'var(--bg-secondary)', color: colors.text }} /></div>
        ...
        <div className="modal-overlay" onClick={() => setRejectingId(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ backgroundColor: colors.bgCard }}>
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label">Reason</label><textarea className="form-textarea" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} required style={{ backgroundColor: 'var(--bg-secondary)', color: colors.text }} /></div>
            </div>
      ...
      <div className="card" style={{ backgroundColor: colors.bgCard }}>
        <div className="card-body p-0">
          {loading ? (<div className="flex items-center justify-center p-8"><div className="loader"></div></div>) : orders.length === 0 ? (
            <div className="text-center py-8" style={{ color: colors.textSecondary }}>No onramp orders found</div>
          ) : (
            <table className="table">
              <thead><tr><th>User</th><th>Fiat</th><th>Crypto</th><th>Fiat Amt</th><th>Crypto Amt</th><th>Method</th><th>Provider</th><th>Status</th><th>Created</th><th>Actions</th></tr></thead>
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td style={{ color: colors.textSecondary }}>{o.paymentMethod}</td>
