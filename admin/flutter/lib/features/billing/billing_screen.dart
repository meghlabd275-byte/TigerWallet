...
class _BillingScreenState extends State<BillingScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final plansRes = await http.get(Uri.parse('http://localhost:9093/api/v1/admin/billing/plans'), headers: {'Authorization': 'Bearer ${await _getToken()}'});
      final subRes = await http.get(Uri.parse('http://localhost:9093/api/v1/admin/billing/subscription'), headers: {'Authorization': 'Bearer ${await _getToken()}'});
      final invoicesRes = await http.get(Uri.parse('http://localhost:9093/api/v1/admin/billing/invoices'), headers: {'Authorization': 'Bearer ${await _getToken()}'});
  ...
  Future<void> _subscribe(String planId) async {
    try {
      await http.post(Uri.parse('http://localhost:9093/api/v1/admin/billing/subscription'), headers: {'Authorization': 'Bearer ${await _getToken()}'}, body: json.encode({'plan_id': planId}));
      _loadData();
