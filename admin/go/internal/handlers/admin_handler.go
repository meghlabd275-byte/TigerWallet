...
func (h *AdminHandler) Login(c *gin.Context) {
	...
	if err := bcrypt.CompareHashAndPassword([]byte(admin.PasswordHash), []byte(hashedPassword)); err != nil {
		...
		// Log failed attempt
		h.logActivity(admin.ID, "login", "admin", strconv.FormatUint(uint64(admin.ID), 10), "Failed login attempt", c.ClientIP(), c.Request.UserAgent(), "failed", err.Error())
	...
	h.logActivity(admin.ID, "login", "admin", strconv.FormatUint(uint64(admin.ID), 10), "Successful login", c.ClientIP(), c.Request.UserAgent(), "success", "")
...
func (h *AdminHandler) Logout(c *gin.Context) {
	...
	if adminID, exists := c.Get("admin_id"); exists {
		h.logActivity(adminID.(uint), "logout", "admin", strconv.FormatUint(uint64(adminID.(uint)), 10), "Admin logged out", c.ClientIP(), c.Request.UserAgent(), "success", "")
	}
...
func (h *AdminHandler) UpdateProfile(c *gin.Context) {
	...
	// Log activity
	h.logActivity(adminID, "update_profile", "admin", strconv.FormatUint(uint64(adminID), 10), "Profile updated", c.ClientIP(), c.Request.UserAgent(), "success", "")
...
func (h *AdminHandler) ChangePassword(c *gin.Context) {
	...
	// Log activity
	h.logActivity(adminID, "change_password", "admin", strconv.FormatUint(uint64(adminID), 10), "Password changed", c.ClientIP(), c.Request.UserAgent(), "success", "")
...
func (h *AdminHandler) CreateAdmin(c *gin.Context) {
	...
	// Log activity
	h.logActivity(adminID, "create_admin", "admin", strconv.FormatUint(uint64(admin.ID), 10), "Created new admin: "+admin.Email, c.ClientIP(), c.Request.UserAgent(), "success", "")
