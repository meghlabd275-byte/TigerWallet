...
pub mod auth {
    ...
    mod tests {
        ...
        fn jwt_sign_decode_roundtrip() {
            ...
            let exp = (chrono::Utc::now() + chrono::Duration::minutes(15)).timestamp();
        ...
        fn require_role_enforced() {
            let c = claims((chrono::Utc::now() + chrono::Duration::minutes(15)).timestamp());
            assert!(require_role(&c, "super_admin").is_ok());
            assert_eq!(require_role(&c, "admin"), Err(StatusCode::FORBIDDEN));
        }
    }
}
