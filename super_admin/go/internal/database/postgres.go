...
func runMigrations(ctx context.Context) error {
	migrations := []string{
		...
		// ---- Trading control-plane (owner policy: SuperAdmin governs every
		// builtin trading vertical; published to shared Redis for the wallet
		// engines to enforce) ----
		`CREATE TABLE IF NOT EXISTS trading_contracts (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), kind VARCHAR(20) NOT NULL, symbol VARCHAR(60) NOT NULL, base_asset VARCHAR(30) NOT NULL, quote_asset VARCHAR(30) NOT NULL, chain_id BIGINT DEFAULT 0, max_leverage INT DEFAULT 1, min_size TEXT DEFAULT '0', tick_size TEXT DEFAULT '0', status VARCHAR(20) DEFAULT 'active', created_by VARCHAR(255), created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE (kind, symbol))`,
		`CREATE TABLE IF NOT EXISTS liquidity_pools (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), chain_id BIGINT NOT NULL, dex VARCHAR(60) NOT NULL, pool_address VARCHAR(255), token0 VARCHAR(80) NOT NULL, token1 VARCHAR(80) NOT NULL, fee_bps INT DEFAULT 30, status VARCHAR(20) DEFAULT 'active', created_by VARCHAR(255), created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE (chain_id, dex, token0, token1))`,
		`CREATE TABLE IF NOT EXISTS margin_markets (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), symbol VARCHAR(60) UNIQUE NOT NULL, base_asset VARCHAR(30) NOT NULL, quote_asset VARCHAR(30) NOT NULL, max_leverage INT DEFAULT 3, borrow_cap TEXT DEFAULT '0', status VARCHAR(20) DEFAULT 'active', created_by VARCHAR(255), created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`,
		`CREATE TABLE IF NOT EXISTS trading_control_audit (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), actor VARCHAR(255), actor_role VARCHAR(50), action VARCHAR(30) NOT NULL, kind VARCHAR(30) NOT NULL, entity VARCHAR(255) NOT NULL, detail TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`,
