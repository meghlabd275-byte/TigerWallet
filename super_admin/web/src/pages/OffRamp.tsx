/**
 * TigerWallet Super Admin - OffRamp Orders Page
...
export default function OffRamp() {
  ...
  const [form, setForm] = useState({
    user_id: '', provider: '', crypto_token: '', fiat_currency: '', crypto_amount: '', fiat_amount: '',
  });
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const res: any = await superAdminApi.getOffRampOrders();
      setItems(res.orders || res.data || res || []);
    } catch (err: any) {
      setError(err?.message || 'Failed to load offramp orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await superAdminApi.createOffRampOrder({
      ...
      setShowForm(false);
      setForm({ user_id: '', provider: '', crypto_token: '', fiat_currency: '', crypto_amount: '', fiat_amount: '' });
      load();
    } catch (err: any) {
      alert(err?.message || 'Failed to create offramp order');
    } finally {
  ...
  const handleReject = async () => {
    ...
    await run(() => superAdminApi.rejectOffRampOrder(rejectId, reason));
    setRejectId(null);
    setReason('');
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-primary mb-6">OffRamp Orders</h1>
      ...
      {showForm && (
        <div className="card mb-6"><div className="card-body">
          <h3 className="text-primary mb-4">Create OffRamp Order</h3>
      ...
      {error ? (
        ...
        <div className="card"><div className="card-body text-center py-8"><p className="text-secondary">No offramp orders found.</p></div></div>
      ) : (
        <div className="card"><div className="card-body overflow-x-auto">
          <table className="table"><thead><tr><th>User</th><th>Provider</th><th>Crypto</th><th>Fiat</th><th>Crypto Amount</th><th>Fiat Amount</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {items.map((o) => (
                <tr key={o.id}>
                  ...
                  <td><div className="flex gap-2">
                    {o.status === 'pending' && <>
                      <button className="btn btn-primary" disabled={actionLoading} onClick={() => run(() => superAdminApi.approveOffRampOrder(o.id))}>Approve</button>
                      <button className="btn btn-danger" disabled={actionLoading} onClick={() => { setRejectId(o.id); setReason(''); }}>Reject</button>
                    </>}
                    <button className="btn btn-secondary" disabled={actionLoading} onClick={() => { if (confirm('Delete this order?')) run(() => superAdminApi.deleteOffRampOrder(o.id)); }}>Delete</button>
                  </div></td>
                </tr>
              ))}
            </tbody>
      ...
      {rejectId && (
        <div className="card mt-4"><div className="card-body">
          <h3 className="text-primary mb-2">Reject OffRamp Order</h3>
