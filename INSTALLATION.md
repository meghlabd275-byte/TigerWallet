# TigerWallet Installation Guide
...
docker run -d \
  ...
  -v tigerwallet-data:/var/lib/postgresql/data \
  -p 5432:5432 \
  postgres:14-alpine

# Run migrations (latest consolidated schema)
psql -h localhost -U postgres -d tigerwallet -f database/schemas/main_schema.sql
```

### 3. Configure Environment
There is **no committed `.env.example`** (env files are git-ignored to prevent
credential leakage). Create `.env` from the variable groups in
[`ENVIRONMENT.md`](ENVIRONMENT.md) and set real values.

### 4. Build & Start
```bash
# Build the canonical UserWallet backend
cd go/wallet_api && go build -o wallet-api .

# Start the platform (all ~42 services)
cd ../.. && docker-compose up -d
```

> The canonical backends and their ports are listed in
...
TigerWallet supports **120 EVM mainnets + 66 non-EVM chains** (see
`go/wallet_api/chains_evm_data.go` and `chains_nonevm_data.go`):

**EVM (120):**
Ethereum, BNB Chain, Polygon, Arbitrum, Optimism, Base, Avalanche, Cronos,
Celo, Gnosis, Fantom, Linea, zkEVM, Scroll, Mantle, opBNB, Moonbeam, Kava,
Harmony, Astar, and more — the authoritative registry is served via
`GET /api/v1/chains`.

**Non-EVM (66):**
Solana, Bitcoin, Cosmos, Sui, Aptos, Tron, Litecoin, Dogecoin, XRP, Near,
Hedera, Algorand, MultiversX, Ton, Celestia, Injective, Sei, Osmosis, Dymension,
and more.
...
- **Documentation:** https://github.com/meghlabd275-byte/TigerWallet
- **Architecture / Security / Environment:** see `ARCHITECTURE.md`, `SECURITY.md`, `ENVIRONMENT.md`
