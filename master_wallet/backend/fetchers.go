...
// fetchers.go — REAL on-chain data fetchers. Every balance/token/gas/price/
// history value comes from a live RPC node, CoinGecko, or an Etherscan-
// compatible explorer. No hardcoded "0", no fabricated prices.
...
func decodeStringOrBytes(data []byte) string {
	...
	// If the data is 64 bytes it may be a bytes32-encoded string; if longer it's
	// a dynamic string with length at offset 32.
	if len(data) >= 64 {
		// dynamic string: first 32 bytes = offset, next 32 = length, rest = data
...
type TokenBalance struct {
	...
	Balance  string  `json:"balance"` // human-readable
...
// FetchTransactionHistory queries an Etherscan-compatible explorer for the
// real on-chain transaction history of an address.
