 ...
 * NEXT_PUBLIC_GOOGLE_DRIVE_CLIENT_ID; fail-closed if unset.
 */
...
const getClientId = (): string => {
  const id =
    (typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_GOOGLE_DRIVE_CLIENT_ID) || '';
  if (!id) {
    throw new Error(
      'Google Drive backup is not configured: set NEXT_PUBLIC_GOOGLE_DRIVE_CLIENT_ID'
    );
  }
  return id;
};
