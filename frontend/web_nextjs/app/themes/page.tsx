...
export default function Themes() {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <header className={`${isDark ? 'bg-gray-800' : 'bg-white border border-gray-200'} border-b p-4`}><div className="flex items-center gap-4"><a href="/wallet" className="text-2xl">🐯</a><h1 className="text-xl font-bold">Theme Customization</h1></div></header>
      <div className="max-w-4xl mx-auto p-8">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">{THEMES.map(t => <button key={t.id} onClick={() => setCurrentTheme(t.id)} className={`p-4 rounded-lg border-2 ${currentTheme === t.id ? 'border-orange-500' : isDark ? 'border-gray-700' : 'border-gray-200'}`}><div className="h-20 rounded-lg mb-2 flex items-center justify-center" style={{ background: t.bg }}><span style={{ color: t.accent, fontSize: 24 }}>🐯</span></div><div className="font-semibold">{t.name}</div></button>)}</div>
