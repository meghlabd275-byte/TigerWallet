...
export default function OrderbookPage() {
  ...
  return (
    <div className={`'min-h-screen' ${isDark ? 'bg-slate-900' : 'bg-slate-50'} ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <select 
              ...
              className={`${isDark ? 'bg-slate-700' : 'bg-slate-100'} 'border' ${isDark ? 'border-slate-600' : 'border-slate-300'} 'rounded-lg px-4 py-2'`}
            >
              {MARKETS.map(m => (
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="grid grid-cols-6 gap-4 mb-4">
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>Last Price</p>
            <p className="text-2xl font-bold">${selectedMarket.price.toLocaleString()}</p>
          </div>
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>24h Change</p>
          ...
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>24h High</p>
            <p className="text-xl font-bold">${selectedMarket.high24h.toLocaleString()}</p>
          </div>
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>24h Low</p>
            <p className="text-xl font-bold">${selectedMarket.low24h.toLocaleString()}</p>
          </div>
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>24h Volume</p>
            <p className="text-xl font-bold">${(selectedMarket.volume24h / 1000000).toFixed(2)}M</p>
          </div>
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-sm'`}>Spread</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          {/* Order Book */}
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4 col-span-2'`}>
            <h3 className="font-bold mb-4">Order Book</h3>
            
            <div className="grid grid-cols-2 gap-4">
              {/* Bids */}
              <div>
                <div className={`'grid grid-cols-3 text-sm' ${isDark ? 'text-slate-400' : 'text-slate-500'} 'mb-2'`}>
                  <span>Price</span>
          ...
          <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
            ...
            <div className={`'flex mb-4' ${isDark ? 'bg-slate-700' : 'bg-slate-100'} 'rounded-lg p-1'`}>
              <button
                onClick={() => setSide('buy')}
            ...
            <div className="space-y-3">
              <div>
                <label className={`'text-sm' ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Price (USDT)</label>
                <input
                  type="number"
                  value={price}
                  ...
                  className={`'w-full' ${isDark ? 'bg-slate-700' : 'bg-slate-100'} 'border' ${isDark ? 'border-slate-600' : 'border-slate-300'} 'rounded-lg p-3 mt-1'`}
                />
              </div>
              <div>
                <label className={`'text-sm' ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Amount ({selectedMarket.base})</label>
                <input
                  type="number"
                  value={amount}
              ...
              <div className={`'p-3' ${isDark ? 'bg-slate-700' : 'bg-slate-100'} 'rounded-lg'`}>
                <div className="flex justify-between text-sm mb-1">
                  <span className={`${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total</span>
                  <span>${(parseFloat(price || '0') * parseFloat(amount || '0')).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className={`${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Fee (0.1%)</span>
        ...
        {/* Open Orders */}
        <div className={`'mt-4' ${isDark ? 'bg-slate-800' : 'bg-white'} 'rounded-lg p-4'`}>
          <h3 className="font-bold mb-4">Open Orders</h3>
          {orders.filter(o => o.status === 'open').length === 0 ? (
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} 'text-center py-8'`}>No open orders</p>
          ) : (
            <div className="space-y-2">
              {orders.filter(o => o.status === 'open').map(order => (
                <div key={order.id} className={`'flex items-center justify-between p-3' ${isDark ? 'bg-slate-700' : 'bg-slate-100'} 'rounded-lg'`}>
                  <div className="flex items-center gap-4">
                    ...
                    <span className={`${isDark ? 'text-slate-400' : 'text-slate-500'}`}>= ${order.total.toFixed(2)}</span>
                  </div>
                  <button
                    onClick={() => handleCancelOrder(order.id)}
                    className={`'px-4 py-1' ${isDark ? 'bg-slate-600' : 'bg-slate-200'} 'rounded hover:bg-slate-500'`}
                  >
                    Cancel
                  </button>
                </div>
