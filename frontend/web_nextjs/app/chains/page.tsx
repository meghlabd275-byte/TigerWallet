    ...
    <div style={{ background: bgPrimary, minHeight: '100vh', color: textColor }}>
      ...
      {selectedChain && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setSelectedChain(null)}>
          <div style={{ background: bgModal, borderRadius: 16, padding: 32, maxWidth: 500, width: '90%', border: `2px solid ${chainColors[selectedChain.name.toLowerCase()] || '#f97316'}` }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, color: textSecondary }}>{selectedChain.type.toUpperCase()} • {selectedChain.symbol}</p>
              </div>
            </div>
            <p style={{ color: textSecondary, margin: '0 0 4px' }}>Chain ID</p>
            <p style={{ margin: '0 0 16px', fontSize: 18 }}>{selectedChain.chainId}</p>
            <p style={{ color: textSecondary, margin: '0 0 4px' }}>Description</p>
