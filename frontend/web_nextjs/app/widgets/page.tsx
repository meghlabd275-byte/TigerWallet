...
export default function Widgets() {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
      <header className={`${isDark ? 'bg-slate-800' : 'bg-white'} border-b p-4`}><div className="flex items-center gap-4"><a href="/wallet" className="text-2xl">🐯</a><h1 className="text-xl font-bold">Widgets</h1></div></header>
      <div className="max-w-2xl mx-auto p-8">
        <div className={`${isDark ? 'bg-slate-800' : 'bg-white border border-gray-200'} rounded-lg p-6 mb-6`}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">{WIDGET_TYPES.map(w => <button key={w.id} onClick={() => toggleWidget(w.id)} className={`p-4 rounded-lg border-2 text-left ${activeWidgets.includes(w.id) ? `border-orange-500 ${isDark ? 'bg-orange-900/20' : 'bg-orange-50'}` : `${isDark ? 'border-slate-700' : 'border-slate-200'}`}`}><div className="text-2xl mb-2">{w.icon}</div><div className="font-semibold">{w.name}</div><div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{w.description}</div></button>)}</div>
        </div>
        <div className={`${isDark ? 'bg-slate-800' : 'bg-white border border-gray-200'} rounded-lg p-6 mb-6`}>
        ...
        {saved && (
          <div className={`${isDark ? 'bg-green-900/30 text-green-400' : 'bg-green-100 text-green-700'} px-4 py-2 rounded-lg mb-4 text-center`}>
