# TigerWallet API Documentation

## Overview

TigerWallet API provides programmatic access to all trading, swapping, and wallet operations. All endpoints return JSON.

**Base URL:** `https://api.tigerwallet.com`

> The API surface described here is implemented by `api_gateway/rest_api`
> (external-platform tiers/register/trade/swap, bot-subscription, admin, and
> white-label routes). The canonical **wallet** APIs live in `go/wallet_api`
> (see `user_wallet/README.md`) and `master_wallet/backend`
> (see `master_wallet/CANONICAL_API_CONTRACT.md`).
...
### Go
```go
import "github.com/tigerwallet/sdk-go"

client := tigerwallet.NewClient("YOUR_API_KEY")
order, err := client.ExecuteTrade(tigerwallet.TradeRequest{
    Platform: "platform_id",
    Symbol: "ETH/USDT",
    Side: "buy",
    Type: "market",
...
const client = new TigerWallet('YOUR_API_KEY')
