...
| 24 | **Smart wallet / ERC-4337 account abstraction (gasless, paymaster)** | Coinbase/MetaMask | 🟡 Canonical base lib present; **`SimpleAccount.sol` + `AccountFactory.sol` now ported with REAL ECDSA validation** (replaced accept-any-64-byte-sig `validateUserOp`); 5 Foundry tests pass. Deployable Paymaster + gasless bundling UI still pending. | 🟡 |
...
| 28 | **Gasless tx / gas-account / paymaster** | MetaMask/Bitget | 🟠 `gasless_tx`, `gas_account`, `paymaster_sdk` — paymaster is the fake (length==64) one; **AA account signature validation is now real** (row 24), but no shipped gasless/gas-subsidy product yet. | 🟠 |
...
Gaps #1, #3, #13, the theme UX gap, the ELF-binary hygiene gap, the NFT-mock-data gap, the keystore-V3-interop gap, the dApp-directory gap, the token-asset-registry gap, the 17-.gitkeep gap, the LICENSE gap, and the AA-AccountFactory port gap are now resolved; items 1, 2, 4, 5, 6, 7, 8 above remain open.
