...
TigerWallet supports **120 EVM blockchain networks** and **66 non-EVM blockchain networks** with full [admin management](./ADMIN_ARCHITECTURE.md) (add, edit, update, remove chains dynamically). These are the verified counts served by the canonical chain registry (`GET /api/v1/chains`).

### EVM Chains (120 supported)
...
| Modules | 100+ |
| Microservices | 250+ |
| Blockchains Supported | 120 EVM, 66 non-EVM |
...
```bash
# Start the full platform (all ~42 services: postgres, redis, backends, frontends)
docker-compose up -d

# Or run one canonical backend directly (requires PostgreSQL + Redis)
cd go/wallet_api && go run .            # UserWallet API :8443
cd master_wallet/backend && go run .    # MasterWallet backend :8450
cd admin/go && go run .                 # Admin API :9093
cd super_admin/go && go run .           # SuperAdmin API :8082
```

> The root `package.json` is only a scaffold (its scripts echo "Run from
> frontend/web_nextjs"). Frontend apps live under `user_wallet/web`,
> `admin/web`, `super_admin/web`, `frontend/web_nextjs`, etc. — build each in
> its own directory (see `BUILD_DEPLOY.md`).
