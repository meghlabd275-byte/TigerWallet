// Package config holds the standalone WL-Liquidity backend configuration.
...
// PostgreSQL-persisted (real liquidity sources + routes) instead of in-memory.
package config
...
type Config struct {
	...
	Product           string // product identifier ("liquidity")
...
func Load() *Config {
	return &Config{
		Port:              getEnv("LIQ_PORT", getEnv("PORT", "8462")),
		DatabaseURL:       getEnv("DATABASE_URL", "postgres://tigerwallet:tigerwallet@localhost:5432/wl_liquidity?sslmode=disable"),
		...
		Product:           getEnv("WL_PRODUCT", "liquidity"),
