/**
 * Offramp — fiat-offramp order governance (approve/reject; no fund movement).
 * Backend: GET/POST /api/v1/admin/offramp, GET/PUT/DELETE /:id, POST /:id/approve, POST /:id/reject {reason}.
 */
...
export default function Offramp() {
  ...
  const load = async () => {
    ...
    try { const d = await whiteLabelAdminApi.getOfframpOrders(); setRows(d.orders || d.offramp || d || []); }
    catch (e: any) { setError(e.message || 'Failed to load offramp orders'); }
    finally { setLoading(false); }
  };

  const approve = async (id: string) => {
    try { await whiteLabelAdminApi.approveOfframpOrder(id); load(); }
    catch (e: any) { setError(e.message || 'Failed to approve'); }
  };

  const doReject = async () => {
    if (!rejectId || !reason) { setError('Reason required'); return; }
    try { await whiteLabelAdminApi.rejectOfframpOrder(rejectId, reason); setRejectId(null); setReason(''); load(); }
  ...
  return (
    <div className="p-6">
      <h1 className={`text-2xl font-bold mb-2 ${cardText}`}>Offramp</h1>
      <p className={`mb-4 ${muted}`}>Fiat-offramp order governance — approve / reject (no fund movement).</p>
        ...
        <div className={`${cardBg} rounded-lg shadow overflow-hidden border ${border}`}>
          <table className="w-full">
            <thead className={thBg}>
              <tr>
                {['User', 'Crypto', 'Amount', 'Fiat', 'Status', 'Actions'].map((h) => (
                  <th key={h} className={`px-4 py-2 text-left text-xs font-semibold ${muted}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className={`divide-y ${border}`}>
              {rows.length === 0 && <tr><td colSpan={6} className={`px-4 py-8 text-center ${muted}`}>No offramp orders.</td></tr>}
              {rows.map((r) => (
                <tr key={r.id}>
                  <td className={`px-4 py-3 ${cardText}`}>{r.user_id}</td>
                  <td className={`px-4 py-3 ${muted}`}>{r.crypto_currency}</td>
                  <td className={`px-4 py-3 ${muted}`}>{r.crypto_amount}</td>
