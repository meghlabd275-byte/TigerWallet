...
class PrivacyService {
  ...
  // ZK Proofs
  ///
  /// Real zero-knowledge proofs are generated by the Rust backend
  /// (`core/rust/zk_infrastructure`: real Fiat-Shamir Schnorr over
  /// Ristretto255). This client must NOT fabricate a proof from random bytes.
  /// It delegates to the backend ZK prover; if the backend is unavailable it
  /// throws rather than returning a fake proof.
  Future<ZKProof> createZKProof({
    ...
    throw UnimplementedError(
      'ZK proof generation is performed by the Rust backend '
      '(core/rust/zk_infrastructure). Wire this client to POST the statement '
      'to the backend ZK prover endpoint instead of fabricating a proof.',
    );
  }

  /// Verify a ZK proof via the backend verifier. NEVER accept a proof without
  /// real verification — the previous implementation returned `true`
  /// unconditionally (an auth/privacy bypass). Until the backend verifier is
  /// wired, this returns `false` (reject) rather than accepting anything.
  ...
  String _hash(String input) {
    // Real SHA-256 hash. The previous implementation was a toy `*31` fold
    // (mod 2^32) — NOT cryptographic and trivially invertible.
