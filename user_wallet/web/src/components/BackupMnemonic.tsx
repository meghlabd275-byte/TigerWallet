...
//      configured via REACT_APP_GOOGLE_CLIENT_ID. If no client ID is set, the
...
export default function BackupMnemonic({ mnemonic, walletId, walletPassword, onConfirmed }: Props) {
  ...
  const initGoogleClient = (): Promise<string> => {
    return new Promise((resolve, reject) => {
      if (!GOOGLE_CLIENT_ID) {
        reject(new Error('Google Drive backup is not configured (REACT_APP_GOOGLE_CLIENT_ID unset).'));
        return;
      }
    ...
    <div className={`backup-card ${isDark ? 'dark' : 'light'}`}>
      ...
      <div className="backup-actions">
        ...
        <button
          ...
          title={GOOGLE_CLIENT_ID ? '' : 'Set REACT_APP_GOOGLE_CLIENT_ID to enable Google Drive backup'}
        >
