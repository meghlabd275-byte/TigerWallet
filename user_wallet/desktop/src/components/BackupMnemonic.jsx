...
//      configured via GOOGLE_CLIENT_ID. If no client ID is set, the button is
//      disabled with an honest message (NEVER a fake success).
...
// Vite exposes VITE_/REACT_APP_-prefixed env on import.meta.env, and also
// defines process.env.* via define in vite.config.js (see web index.html
// script tags). Guard both access paths so the renderer never throws when env
// is absent — the button is then honestly disabled.
const GOOGLE_CLIENT_ID =
  ...
  (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.REACT_APP_GOOGLE_CLIENT_ID) ||
  '';

export default function BackupMnemonic({ mnemonic, walletId, walletPassword, onConfirmed }) {
  ...
  const initGoogleClient = () => {
    return new Promise((resolve, reject) => {
      if (!GOOGLE_CLIENT_ID) {
        reject(new Error('Google Drive backup is not configured (GOOGLE_CLIENT_ID unset).'));
        return;
      }
      ...
      // Load the gapi client + drive discovery doc.
  ...
  const uploadToGoogleDrive = (accessToken) => {
    ...
    <div className={`backup-card ${isDark ? 'dark' : 'light'}`}>
      ...
      <div className="backup-actions">
        ...
        <button
          ...
          title={GOOGLE_CLIENT_ID ? '' : 'Set GOOGLE_CLIENT_ID to enable Google Drive backup'}
        >
