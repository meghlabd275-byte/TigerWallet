...
//
// Mirrors web/src/components/TxSubmittedBanner.tsx, adapted to the production
...
export default function TxSubmittedBanner({ txHash, chainId, onDismiss }: Props) {
  ...
  return (
    <div className={`tx-submitted-banner ${isDark ? 'dark' : 'light'}`}>
      ...
      <button className="banner-close" onClick={() => { setVisible(false); onDismiss?.(); }} aria-label="Dismiss">
        ×
      </button>
    </div>
  );
