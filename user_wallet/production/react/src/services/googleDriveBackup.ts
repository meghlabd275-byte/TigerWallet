 ...
 * REACT_APP_GOOGLE_DRIVE_CLIENT_ID with a fallback to
 * VITE_GOOGLE_DRIVE_CLIENT_ID; fail-closed if neither is set.
 */
...
const getClientId = (): string => {
  // CRA defines process.env.* at build time; Vite exposes import.meta.env.*.
  // Read defensively so this type-checks without @types/node installed.
  const g = globalThis as unknown as {
    process?: { env?: Record<string, string | undefined> };
  };
  const env = g.process && g.process.env ? g.process.env : {};
  const id =
    env.REACT_APP_GOOGLE_DRIVE_CLIENT_ID || env.VITE_GOOGLE_DRIVE_CLIENT_ID || '';
  if (!id) {
    throw new Error(
      'Google Drive backup is not configured: set REACT_APP_GOOGLE_DRIVE_CLIENT_ID or VITE_GOOGLE_DRIVE_CLIENT_ID'
    );
  }
  return id;
};
