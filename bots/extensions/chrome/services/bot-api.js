// TigerBots Chrome Extension API client.
//
// Targets the standalone Bots backend (mm_bot_platform/bot_api, port 8471,
// path prefix /api/v1). JWT bearer auth; the token is persisted in
// chrome.storage.local under `tigerbots-token`. Base URL is configurable via
// the `botsApiUrl` storage entry and defaults to the local dev endpoint.
//
...
// Chrome MV3 service worker: chrome.storage.local is async and survives worker
// restarts. The in-memory cache is best-effort.
