/**
 * Transactions Page
 * View all transactions across white labels
 */
...
export default function Transactions() {
  ...
  const loadTransactions = async () => {
    try {
      const data = await masterAdminApi.getTransactions({ pageSize: 50 });
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {transactions.map((tx) => (
              <tr key={tx.id}>
                ...
                <td className="px-6 py-4 text-sm font-mono">{tx.hash?.substring(0, 10)}...</td>
                <td className="px-6 py-4 text-sm font-mono">{tx.from?.substring(0, 10)}...</td>
                <td className="px-6 py-4 text-sm font-mono">{tx.to?.substring(0, 10)}...</td>
