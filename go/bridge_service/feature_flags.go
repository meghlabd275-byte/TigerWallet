// feature_flags.go — downstream feature-flag enforcement layer for the bridge
// service.
//
...
// redisClientForFlags returns the bridge service Redis client used to read flag
// state. Returns nil when the service / Redis is not initialized (fail-closed
// downstream).
func (bs *BridgeService) redisClientForFlags() *redis.Client {
	if bs == nil || bs.redis == nil {
		return nil
	}
	return bs.redis
}
...
func (bs *BridgeService) FeatureState(featureName string) string {
	if featureName == "" {
	...
	state := bs.fetchFeatureState(featureName)
...
func (bs *BridgeService) fetchFeatureState(featureName string) string {
	rdb := bs.redisClientForFlags()
	if rdb == nil {
		return flagStateDisabled
	}
...
func (bs *BridgeService) IsFeatureEnabled(featureName string) bool {
	return bs.FeatureState(featureName) == flagStateEnabled
}
...
func (bs *BridgeService) InvalidateFeatureCache(featureName string) {
	flagCacheMu.Lock()
...
//
//	if !bs.enforceFeature(c, FeatureBridge) { return }
func (bs *BridgeService) enforceFeature(c *gin.Context, featureName string) bool {
	if bs.IsFeatureEnabled(featureName) {
		return true
	}
	state := bs.FeatureState(featureName)
	if state == "" {
