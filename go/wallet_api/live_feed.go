...
type liveFeedHub struct {
	...
	// rdb, when set, enables cluster-wide coordination: ONE replica fetches
	// upstream per tick (distributed lock) and every replica serves its
	// subscribers from the shared Redis cache — N replicas x M clients still
	// cost exactly 1 upstream call per tick.
	rdb *redis.Client
...
func (h *liveFeedHub) tickLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			if h.rdb != nil {
				h.fetchAndBroadcastCluster(symbols)
			} else {
...
// fetchUpstreamTickers performs ONE batched upstream markets call for every
// subscribed symbol and returns symbol -> ticker frame. On failure it returns
// an error; callers decide how to surface it (error frames) and nothing is
// ever fabricated.
func (h *liveFeedHub) fetchUpstreamTickers(symbols []string) (map[string]gin.H, error) {
	...
	if err != nil || resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("upstream markets call failed: status=%d err=%v", resp.StatusCode, err)
	}
...
// broadcastError notifies every subscriber of the given symbols that the feed
// is temporarily unavailable (fail-closed; no price is fabricated).
func (h *liveFeedHub) broadcastError(symbols []string) {
	for _, sym := range symbols {
		h.broadcast(sym, gin.H{"type": "error", "symbol": sym, "message": "price feed unavailable"})
	}
}

// fetchAndBroadcast is the single-replica path (no Redis): fetch upstream and
// fan out locally.
func (h *liveFeedHub) fetchAndBroadcast(symbols []string) {
	tickers, err := h.fetchUpstreamTickers(symbols)
	if err != nil {
		h.broadcastError(symbols)
		return
	}
...
const (
	liveFeedTickerKeyPrefix = "livefeed:ticker:"
	liveFeedFetchLockKey    = "livefeed:fetchlock"
)

// fetchAndBroadcastCluster is the clustered path: exactly ONE replica in the
// fleet fetches upstream per tick (Redis SET NX lock), writes each ticker to
// the shared cache, and every replica (including the fetcher) serves its own
// subscribers from that cache. Result: a planet-scale fleet still issues ONE
// upstream provider call per tick, and a replica failover just moves the lock.
func (h *liveFeedHub) fetchAndBroadcastCluster(symbols []string) {
	interval := liveFeedInterval()
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	lockTTL := interval * 4 / 5
	if lockTTL < 500*time.Millisecond {
		lockTTL = 500 * time.Millisecond
	}
	acquired, err := h.rdb.SetNX(ctx, liveFeedFetchLockKey, clusterNodeID(), lockTTL).Result()
	if err == nil && acquired {
		// This replica is the designated fetcher for this tick.
		tickers, ferr := h.fetchUpstreamTickers(symbols)
		if ferr != nil {
			h.broadcastError(symbols)
			return
		}
		...
		for sym, t := range tickers {
			if data, merr := json.Marshal(t); merr == nil {
				pipe.Set(wctx, liveFeedTickerKeyPrefix+sym, data, cacheTTL)
			}
		}
		_, _ = pipe.Exec(wctx)
		for sym, t := range tickers {
	...
	// Another replica holds the fetch lock: serve from the shared cache.
	keys := make([]string, 0, len(symbols))
	for _, sym := range symbols {
		keys = append(keys, liveFeedTickerKeyPrefix+sym)
	}
	vals, verr := h.rdb.MGet(ctx, keys...).Result()
	if verr != nil {
		return // cache read failed; subscribers get the next tick
	}
	for i, raw := range vals {
		s, ok := raw.(string)
		if !ok || s == "" {
			continue // fetcher has not written this symbol yet this tick
		}
		var frame gin.H
		if json.Unmarshal([]byte(s), &frame) == nil {
			h.broadcast(strings.TrimPrefix(keys[i], liveFeedTickerKeyPrefix), frame)
		}
	}
}
