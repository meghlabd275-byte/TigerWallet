...
// fiat_ramp is replaced per-service with the canonical feature flag name.
const GatedFeature = "fiat_ramp"
...
func (s *FiatRampService) fetchFeatureState(featureName string) string {
...
func (s *FiatRampService) FeatureState(featureName string) string {
	if featureName == "" {
		return flagStateDisabled
	}
	now := time.Now()
...
func (s *FiatRampService) isFeatureEnabled(featureName string) bool {
...
func (s *FiatRampService) enforceFeature(c *gin.Context, featureName string) bool {
...
func (s *FiatRampService) featureGate(featureName string) gin.HandlerFunc {
