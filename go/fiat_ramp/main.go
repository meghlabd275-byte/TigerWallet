...
func (s *FiatRampService) CreateOrder(userID, providerID, amount, fiatCurr, cryptoCurr, recipient, payMethod string) (*Order, error) {
	...
	order := &Order{
		ID:     fmt.Sprintf("ramp-%d-%s", time.Now().Unix(), randomStr(8)),
...
func (s *FiatRampService) RegisterRoutes(r *gin.Engine) {
	...
	api.POST("/order", func(c *gin.Context) {
		...
		order, err := s.CreateOrder(req.UserID, req.ProviderID, req.Amount, req.FiatCurrency, req.CryptoCurrency, req.RecipientAddress, req.PaymentMethod)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
