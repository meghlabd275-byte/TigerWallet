...
const FeatureCopyTrading = "copy_trading"
...
// redisClientForFlags returns the copy-trading service Redis client used to
// read flag state. Returns nil when the service / Redis is not initialized
// (fail-closed downstream).
func (s *service) redisClientForFlags() *redis.Client {
...
func (s *service) InvalidateFeatureCache(featureName string) {
	flagCacheMu.Lock()
...
//
//	if !s.enforceFeature(c, FeatureCopyTrading) { return }
