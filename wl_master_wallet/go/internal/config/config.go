// Package config holds the standalone WL-MasterWallet backend configuration.
// This backend runs INDEPENDENTLY in the WL client's own cloud — own PG, own
// signing, own DB. It phones home to the license control plane (TWO_PARTY_GATE_URL)
// on heartbeat and gates every request fail-closed via wlgate.
package config
...
type Config struct {
	...
	Product           string // product identifier ("master_wallet")
...
func Load() *Config {
	return &Config{
		...
		DatabaseURL:       getEnv("DATABASE_URL", "postgres://tigerwallet:tigerwallet@localhost:5432/wl_masterwallet?sslmode=disable"),
		...
		Product:           getEnv("WL_PRODUCT", "master_wallet"),
