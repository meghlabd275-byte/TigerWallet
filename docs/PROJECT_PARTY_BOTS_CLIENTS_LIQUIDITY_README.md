...
| [`GAPS.md`](GAPS.md) | **Authoritative consolidated gap register** — resolved items (stubs-to-PostgreSQL conversion, orphan deletions, port/JSON-contract fixes, admin login wiring) **and** the remaining open items (billing seeds, `go/full_fetchers` scaffold, `selfhosted_masterwallet` gate, sharding plan, Solidity audit) |
| [`FEATURE_GAP_REPORT.md`](FEATURE_GAP_REPORT.md) | Direct answers to the 5-point engineering directive: UserWallet / Admin / MasterWallet / White-Label / ProjectParty+Bots feature+fetcher+gap matrix |
